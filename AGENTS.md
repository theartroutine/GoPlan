...
## 11. Explain Document Standard

When the user asks for an explanation:
- Create a file under `report/<topic>.md` in Vietnamese.
- Use very high detail so a mid/low-level reader can follow.
- Required structure: objective, related files, high-level flow, line-by-line detail, cross-file call chain, done/not done, security implications, test checklist, last updated date.
- If the user asks for a short explanation in chat only, do that instead.
